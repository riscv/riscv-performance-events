...
| <cache> | Instructions for which the data access hit in the selected cache.  See cache naming standards in <<caches-table>>.
| <cache>.MISS | Instructions for which the data access missed in the selected cache.  See cache naming standards in <<caches-table>>.
| <cache>.MERGE | Instructions for which the data access merged with an outstanding miss in the selected cache.  See cache naming standards in <<caches-table>>.
...
[cols="20%,80%"]
|===
| L__x__D | Level _x_ data cache
| L__x__I | Level _x_ instruction cache
| L__x__ | Level _x_ unified cache
|===

For each cache level _x_ for which events are implemented, it is expected that an implementation will define events for either a unified (L__x__) cache, or some combination of split caches (L__x__D and/or L__x__I).  The cache level _x_ is an integer value from 1 to some number _N_, for a system that implements events for at least __N__+1 caches.  The last level cache in the system is a special case, referred to as "LL".  If events are implemented for cache level __x__, it is expected that events are implemented for 1..__x__-1 as well.  

NOTE: _For an implementation with split L1 caches, and unified L2 and L3 caches, the corresponding events would use cache names L1I, L1D, L2, and LL.  If the last level cache is not unified, LL events should not be implemented._
...
A reference example of the full set of CACHE events, for an implementation with split L1 caches and unified L2 and L3 caches, is illustrated in the table below.
...
The top level of the TLB event hierarchy identifies the level of the TLB associated with the event.  The naming scheme matches that for caches, see <<caches-table>>.

NOTE: _For an implementation with split L1 TLBs (a.k.a., DTLB and ITLB) and a unified L2 TLB, the corresponding events would use TLB names L1I, L1D, and LL._
...
A reference example of the full set of TLB events, for an implementation with split L1 TLBs and a unified L2 TLB, is illustrated in the tables below.
...
The instruction fetch data source events leverage a cache naming scheme similar to that described in <<caches-table>>.  However, the I/D modifier for split caches is omitted, since it is implied that these fetch events will access the I-side of any split caches.

.DSRC Event Instruction Fetch Data Sources
[[fetch-dsrcs]]
[cols="20%,80%"]
|===
| <cache> | Instruction fetches for which the instruction data access hit in the specified cache.  See cache names in <<caches-table>>.
