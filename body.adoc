...
This group contains events that count instruction fetch requests, broken down by the address translation source or the instruction data source.  Each event in this group has both speculative (.SPEC) and non-speculative (.RET) versions, though only the speculative events are listed in the tables below.  In the case of non-speculative fetch events, the counter is incremented only when the first instruction in the effected fetch line retires.

NOTE: _Non-speculative fetch events may allow precise attribution; however, non-speculative fetch events are among the most expensive to implement, as they require staging fetch event indications throughout the hart pipeline.  It is recommended that all implementations support the speculative fetch events, while those that are able additionally support the non-speculative fetch events._
...
image::images/tma-full3.svg[TMA Hierarchy]
...
The Frontend Bound state is broken down into Frontend Latency and Frontend Bandwidth L2 states.  Slots that fall into Frontend Latency are cases where the decoders are awaiting fetch data, and hence have no uops to deliver to the backend.  Frontend Latency is broken down into three standard L3 states: Address, Data, and Resteer.  Frontend Address Latency represents slots unfilled due to awaiting the address translation for a fetch line; e.g., due to an ITLB miss.  Frontend Data Latency represents slots unfilled due to awaiting data return for a fetch line; e.g., due to an L1I cache miss.  Frontend Resteer Latency counts frontend pipeline slots discarded due to fetch resteers that can result from control transfers; e.g., due to jumps or taken branches.
...
Bits 4:0 in `mhpmevent__X__` comprise the SEL field, which selects the performance event to be counted by `mhpmcounter__X__`.
