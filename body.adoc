...
The memory access instruction categories are listed below.

.Memory Instruction Categories
[cols="20%,80%"]
|===
| LOAD | Instructions that perform one or more load operations.
| STORE | Instructions that perform one or more store operations.
| LDST | Instructions that perform one or more load or store operations.
|===
...
These events include additional levels of hierarchy that allow counting per data source, address source, or cacheability.  When no additional qualifier is included in the event name (e.g., LOAD.RET), all operations of that type are counted.

.Memory Event Categories
[cols="20%,80%"]
|===
| UC | Uncacheable memory accesses.
| DSRC | Load and store operations that accessed data from the specified source (see <<dsrcs>>).
| ASRC | Load and store operations whose data address translation came from the specified source (see <<asrcs>>).
| DMISS | Load and store operations whose data access missed the specified source.
| AMISS | Load and store operations whose data address translation missed the specified source.
|===

The standard data sources associated with DSRC events are listed in the table below.

.DSRC Event Data Sources
[[dsrcs]]
[cols="20%,80%"]
|===
| STLF | Loads to which data was forwarded by an older store.
| <cache> | Load and store operations that hit in the selected cache.  See cache naming standards in <<caches-table>>.
| SLC | Load and store operations that hit in the system-level cache.
| LOCAL.MEM | Load and store operations that hit in local memory.
| REMOTE.MEM | Load and store operations that hit in remote memory.
| REMOTE.CACHE | Load and store operations that hit in a remote cache.
| REMOTE.HITM | Load and store operations that hit modified data in a remote cache.
|===

The system-level cache (SLC) is the last level of cache within the system.  In some implementations this may be the same as the LL, L2, or even L1D/L1I (see <<caches-table>>).  In such implementations, the SLC events simply alias to the corresponding LL, L2, or L1D/L1I events.
...
NOTE: _Implementations with data sources not included among the standard sources listed above may opt to include additional, custom sources within the DSRC hierarchy._

The standard address sources associated with ASRC events are listed in the table below.

.ASRC Event Translation Sources
[[asrcs]]
[cols="20%,80%"]
|===
| <tlb> | Loads and stores for which the data address translation hit in the selected TLB.  See TLB naming standards in <<TLB Events>>.
| WALK | Loads and stores for which the data address translation required a page walk.
|===

The standard data sources associated with DMISS events are the <cache> and SLC sources listed in <<dsrcs>>.  

The standard address source associated with AMISS events is the L1 TLB source listed in <<asrcs>>.  

NOTE: _DMISS/AMISS may be implemented as pseudo-events, or combinations of DSRC/ASRC events.  For example, LOAD.DMISS.SLC.RET may be implemented as the union of the LOAD.DSRC.LOCAL.MEM.RET, LOAD.DSRC.REMOTE.MEM.RET, LOAD.DSRC.REMOTE.CACHE.RET, and LOAD.DSRC.REMOTE.HITM.RET events._

NOTE: _There are no AMISS.LL events defined because they would simply be aliases of the ASRC.WALK events._
...
This group contains events for instruction, data, and unified caches.  Events in this group count accesses from the cache's perspective; if an instruction performs two cache accesses, they will be counted separately.  All events in this group count speculative accesses.

The top level of the CACHE event hierarchy identifies the cache associated with the event.  The caches with standard events associated are listed below.
...
| LL | Last level of the cache hierarchy for which events can be counted by the hart
|===

In some implementations, the last level that can be counted within the hart may be the L2 or even the L1D/L1I.  In such cases, LL events simply alias to the corresponding L2 or L1D/L1I events.

NOTE: _Implementations with caches that are not represented by the standard caches listed above may opt to include additional, custom events within the CACHE hierarchy._
...
NOTE: _Like all events in this spec, these CACHE events are per-hart.  For caches that may be shared by multiple harts, the lookup events (RD*, WR, RW) should increment only for requests from the local hart.  FILL and WB events should increment only for fills and writebacks resulting from requests from the local hart.  It is up to implementations whether and how to count PREF, SNOOP, and OPREF events for shared caches._
...
The full set of CACHE events is illustrated in the table below.
...
This group contains events for translation lookaside buffers (TLBs).  Events in this group count accesses from the TLB's perspective; if an instruction performs two address translations, they will be counted separately.  All events in this group count speculative accesses.

The top level of the TLB event hierarchy identifies the level of the TLB associated with the event.  
...
In some implementations the L1D or L1I TLBs may be the last (only) level of TLB.  In such implementations, the LL TLB events alias to the L1D/L1I TLB events.
...
The full set of TLB events is illustrated in the tables below.
...
| DMISS | Instruction fetches whose data access missed the specified source.
| AMISS | Instruction fetches whose data address translation missed the specified source.
|===

The standard address and data sources for instruction fetch events are listed below.

.DSRC Event Instruction Fetch Data Sources
[[fetch-dsrcs]]
[cols="20%,80%"]
|===
| <cache> | Instruction fetches that hit in the selected cache.  See cache naming standards in <<caches-table>>.
| SLC | Instruction fetches that hit in the system-level cache.
| LOCAL.MEM | Instruction fetches that hit in local memory.
| REMOTE.MEM | Instruction fetches that hit in remote memory.
| REMOTE.CACHE | Instruction fetches that hit in a remote cache.
| REMOTE.HITM | Instruction fetches that hit modified data in a remote cache.
|===
...
| WALK | Instruction fetches for which the address translation required a page walk.
|===

The standard data sources associated with FETCH.DMISS events are the <cache> and SLC sources listed in <<fetch-dsrcs>>.  

The standard address source associated with FETCH.AMISS events is the L1I TLB source listed in <<fetch-asrcs>>.  
...
Similarly, Memory Data Bound is broken down into L4 states per implemented cache (see <<caches-table>>), plus the External Memory state.  The External Memory State represents slots where the backend is stalled due to awaiting data for requests that missed the last level cache. 
...
| 1 | CYCLES.HART
| 2 | INST.RET
| 3 | CACHE.L1D.RD.DATA.ACCESS
| 4 | CACHE.L1D.RD.DATA.MISS
| 5 | INST.BRJMP.SPEC
| 6 | INST.MISPRED.SPEC
| 7 | Reserved, no event
| 8 | Reserved, no event
...
| 15 | TLB.L1D.LDST.ACCESS
| 16 | TLB.L1D.LDST.MISS
...
| CACHE_REFERENCES | 3 (CACHE.L1D.RD.DATA.ACCESS)
| CACHE_MISSES | 4 (CACHE.L1D.RD.DATA.MISS)
...
| CACHE_L1D_READ_ACCESS | 3 (CACHE.L1D.RD.DATA.ACCESS)
| CACHE_L1D_READ_MISS | 4 (CACHE.L1D.RD.DATA.MISS)
...
| CACHE_DTLB_READ_ACCESS | 15 (TLB.L1D.LDST.ACCESS)
| CACHE_DTLB_READ_MISS | 16 (TLB.L1D.LDST.MISS)
...
Unlike the cache misses example above, which included speculative cache misses, this formula counts only non-speculative mispredictions (that is mispredictions on branches and/or jumps that ultimately retire).
