...
[cols="20%,80%"]
|===
| TK | All JAL, JALR, and taken branch instructions.
...
| RETURN_CORSWAP | The union of RETURN and CORSWAP instructions.
| CALL_CORSWAP | The union of CALL and CORSWAP instructions.
| UNCOND | All JAL and JALR instructions.
|===
...
[cols="20%,80%"]
|===
| <tlb> | Instructions for which the data address translation hit in the selected TLB.  See TLB naming standars in <<TLB Events>>.
| <tlb>.MISS | Instructions for which the data address translation missed in the selected TLB. See TLB naming standars in <<TLB Events>>. 
|===
...
.INST Metrics
include::adoc_event_tables/inst_metrics.adoc[]
...
The top level of the TLB event hierarchy idenfies the level of the TLB associated with the event.  The naming scheme matches that for caches, see <<CACHE Events>>.
...
A reference example of the full set of TLB events and metrics, assuming the example set of TLBs shown above in <<tlbs-ex>>, is illustrated in the tables below.

.TLB Events
include::adoc_event_tables/tlb.adoc[]

.TLB Metrics
include::adoc_event_tables/tlb_metrics.adoc[]
...
[cols="20%,80%"]
|===
| <tlb> | Instruction fetches for which the address translation hit in the selcted TLB.  See TLB naming standars in <<TLB Events>>.
| <tlb>.MISS | Instruction fetches for which the address translation missed in the selected.  See TLB naming standars in <<TLB Events>>.
|===
