...
The table below provides the list of standard event selector encodings and the associated events.  For all other standard events, the event selector encoding is implementation-defined.
...
[NOTE]
====
_The standard encodings above are intended for use with Linux perf legacy events.  The recommended mappings follow.  Not all perf legacy events have mappings listed, for those 0 (no event) should be used._

.Linux Perf Hardware Events
[options="header",width=50%,cols="80%,20%"]
|===
| Perf HW Event | Enc 
| CPU_CYCLES | 1
| INSTRUCTIONS | 2
...
| CACHE_L1D_READ_ACCESS | 3
| CACHE_L1D_READ_MISS | 3
| CACHE_L1I_READ_ACCESS | 11
| CACHE_L1I_READ_MISS | 12
| CACHE_LL_READ_ACCESS | 13
| CACHE_LL_READ_MISS | 14
| CACHE_DTLB_READ_ACCESS | 15
| CACHE_DTLB_READ_MISS | 16
| CACHE_ITLB_READ_ACCESS | 17
| CACHE_ITLB_READ_MISS | 18
| CACHE_BPU_READ_ACCESS | 5
| CACHE_BPU_READ_MISS | 6
|===
====
...
NOTE: _Confusingly, the "misses" in MPKI can refer to cache misses in some contexts, and branch/jump mispredicts in others._
