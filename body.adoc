...
This group contains events that count RISC-V instructions.  Non-speculative events, counted at instruction retirement, are appended with .RET.  Speculative events, which may include events incurred by instructions that do not retire, are appended with .SPEC.  Each event described below has both a RET and a SPEC version.
...
| MISPRED | Branch and jump instructions that were mispredicted.  Includes all BRANCH, JAL, and JALR opcodes, including compressed varieties.
...
| <cache> | Instructions for which the data access hit in the selected cache.  See cache naming standards in <<CACHE>>.
| <cache>.MISS | Instructions for which the data access missed in the selected cache.  See cache naming standards in <<CACHE>>.
| <cache>.MERGE | Instructions for which the data access merged with an outstanding miss in the selected cache.  See cache naming standards in <<CACHE>>.
