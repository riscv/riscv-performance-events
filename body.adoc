...
NOTE: _The standard `mhpmevent__X__`.SEL field is only wide enough to hold the standard encodings, defined below.  An implementation that implements additional events may use additional WARL bits in `mhpmevent__X__` to select events, effectively widening the SEL field.  It is recommended that such implementations use bits adjacent to SEL for this purpose.  For example, for a 12-bit SEL field, use `mhpmevent__X__`[11:0].  In such implementations, the standard encodings should apply when all added SEL bits (bits 11:5 in the example) are 0._
...
[NOTE]
====
_The standard encodings above are intended for use with Linux perf legacy events.  The recommended mappings follow.  Not all perf legacy events have mappings listed, for those without mappings 0 (no event) should be used._
